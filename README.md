...
- **SharedMemory/** – библиотека для работы с разделяемой памятью (`BaseMemory.cpp`, `BaseMemory.h`).
  ...
  - `include/`
    - `BaseMemory.hpp` и `Daemonizer.hpp` - хедеры

- **SubscriptionAdapter/** – адаптер для механизма подписок.
...
- При старте загружает подписки из файла (опция `--sub-file`, по умолчанию `adapter_subscriptions.json`).
- При получении команды `sub_to` / `unsub_to` (тег `"direct"`) обновляет `map<string, set<string>> subscription` и **сразу сохраняет** изменения в JSON.
- При завершении работы (по сигналу) также **сохраняет** текущее состояние подписок.
- Формат JSON: `{"topic1": ["sub1","sub2"], "topic2": []}`.
